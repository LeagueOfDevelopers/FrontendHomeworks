...
LodShop.service('ProductsService', ['JSONService', function (JSONService) {
  ...
  this.SerializeProductJSON = function() {
      var serializedJSON = JSON.parse(productsJSON);
...
LodShop.controller('ProductsController', ['$scope','ProductsService', function ($scope, ProductsService) {
    $scope.products = ProductsService.SerializeProductJSON();
    $scope.remove = function(specification){
        console.log(specification);
        return (specification.piecesInStock > 0);
    };

    $scope.order = function(specification){
        specification.piecesInStock--;
    }
}]);
